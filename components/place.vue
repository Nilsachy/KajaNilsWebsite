            ...
            <v-list-item-group>
              <v-list-item v-for="(place, i) in places" :key="i">
                ...
                <v-list-item-content>
                  ...
                  <v-list-item-subtitle>Added on: {{ place.date }}</v-list-item-subtitle>
                </v-list-item-content>
                <v-btn
                  fab
                  ripple
...
export default {
  ...
  methods: {
    getAllPlaces () {
      axios.get('http://35.180.251.134:8765/backend-kn/places', {
      })
        .then((response) => {
    ...
    addNewPlace () {
      ...
      axios({
        url: 'http://35.180.251.134:8765/backend-kn/places/create',
        method: 'POST',
        data: formdata
      }).then((response) => {
    ...
    editPlace (place) {
      ...
      axios({
        url: 'http://35.180.251.134:8765/backend-kn/places/edit_place',
        method: 'PUT',
        headers: {
    ...
    removePlace (place) {
      ...
      formdata.append('id', place.id)
      axios({
        url: 'http://35.180.251.134:8765/backend-kn/places/delete_place',
        method: 'DELETE',
        headers: {
          'Content-Type': 'multipart/form-data'
        },
